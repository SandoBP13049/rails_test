...
require 'set'
require 'depq.rb'
#require ‘C:/Ruby23-x64/lib/ruby/gems/2.3.0/gems/depq-0.7/lib/depq.rb’
  ...
  def build(csp,double_array,double_array_reverse,matrix)
    ...
    csp.vars.each do |var|
      ...
      results.each do |r|
		...
		#puts "ranks => #{ranks}"
        str = words.shift
		#puts "ranks.zip(vars)=>#{ranks.zip(vars)}"
        ranks = ranks.zip(vars).map{|rank, var| 
		#puts "csp.odoriji?(var) => #{csp.odoriji?(var)}"
		if csp.odoriji?(var) 
		#puts "csp.odoriji[var][1] => #{csp.odoriji[var][1]}" 
		end
		csp.odoriji?(var) ? csp.odoriji[var][1] : rank}
        if words.empty?
